...
> [**LCS의 첫 번째 속성**]
>
> 모든 문자열 $X$, $Y$, 그리고 모든 문자 $A$에 대해서, $\mathbin{\text{\textasciicircum}}$를 문자열 접합이라 정의하였을 때, 다음이 성립한다.
> 
> $$
> LCS(X \mathbin{\text{\textasciicircum}} A, Y \mathbin{\text{\textasciicircum}} A) = LCS(X, Y) \mathbin{\text{\textasciicircum}} A
> $$
...
$$
\begin{align}
LCS(\verb|BANANA|, \verb|ATANA|) &= LCS(\verb|BANAN|, \verb|ATAN|)\mathbin{\text{\textasciicircum}}\verb|A| \\
&= LCS(\verb|BANA|, \verb|ATA|)\mathbin{\text{\textasciicircum}}\verb|NA| \\
&= LCS(\verb|BAN|, \verb|AT|)\mathbin{\text{\textasciicircum}}\verb|ANA|
\end{align}
$$
...
> [**LCS의 두 번째 속성**]
>
> 만일 $A$와 $B$가 서로 다른 기호라면, 모든 문자열 $X$와 $Y$에 대해, $LCS(X\mathbin{\text{\textasciicircum}}A, Y\mathbin{\text{\textasciicircum}}B)$는 집합 $\{ LCS(X\mathbin{\text{\textasciicircum}}A, Y), LCS(X, Y\mathbin{\text{\textasciicircum}}B) \}$에 속하는 가장 길이가 긴 문자열 중 하나이다.

좀더 쉽게 말하면, 두 문자열이 다른 문자로 끝난다면, 각각 한쪽을 지우고 LCS를 구했을 때, 두 LCS중 더 긴 쪽이 LCS라는 것이다.

예를 들어, $LCS(\verb|ABCDEFG|, \verb|BCDGK|)$를 구한다고 해보자.

위 속성에 따라, $LCS(\verb|ABCDEFG|, \verb|BCDGK|) = LCS(\verb|ABCDEF|\mathbin{\text{\textasciicircum}}\verb|G|, \verb|BCDG|\mathbin{\text{\textasciicircum}}\verb|K|)$는,  
$LCS(\verb|ABCDEF|\mathbin{\text{\textasciicircum}}\verb|G|, \verb|BCDG|)$ 와 $LCS(\verb|ABCDEF|, \verb|BCDG|\mathbin{\text{\textasciicircum}}\verb|K|)$ 중 더 긴 쪽의 문자열이며, 두 문자열의 길이가 같다면 그 중 아무거나 선택해도 괜찮다.
...
\text{속성 1) }LCS(X_{i-1}, Y_{j-1})\mathbin{\text{\textasciicircum}}x_i &\text{if } i, j > \text{ and } x_i = y_j \\
