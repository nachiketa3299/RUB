...
결론부터 말하자면, <mark>**싱글턴 패턴은 안티 패턴이 아니다.**</mark>
...
그리고 결정적으로 <mark>"싱글턴을 안 쓸 거면, 대안이 뭔데?"라는 질문에 썩 만족스러운 해답을 가져올 수가 없다.</mark> 애초에 싱글턴을 안 쓰는 프로그램을 만드는 것이 매우 어렵다. **필요악이라고 볼 수도 있지만 싱글턴은 명백히 쓰임새가 있으며 이러한 패턴은 실무에서 효율적이고 재사용 가능함이 오래도록 입증되어 왔다.**
