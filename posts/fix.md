...
>   * `ch`가 *피연산자*라면, 출력한다.
>   * `ch`가 *연산자*라면:
>     * 현재 스택의 탑에 무언가 존재하고, 그게 `(`가 아닌 연산자일 때, <mark>그 연산자의 우선순위가 `ch`의 우선순위 **이상**인 경우, 계속해서</mark> 스택의 탑을 출력하고 팝 한 뒤, `ch`를 푸시한다.
...
![](/media/fix/assets/SY/00.png "0) 시작 단계")
![](/media/fix/assets/SY/01.png "1) A는 피연산자이므로 바로 출력")
![](/media/fix/assets/SY/02.png "2) *는 연산자 이므로 스택에 저장")
![](/media/fix/assets/SY/03.png "3) (는 여는 괄호로, 스택에 저장")
![](/media/fix/assets/SY/04.png "4) B는 피연산자이므로 바로 출력")
![](/media/fix/assets/SY/05.png "5) +는 연산자 이므로 스택에 저장")
![](/media/fix/assets/SY/06.png "6) C는 피연산자이므로 바로 출력")
![](/media/fix/assets/SY/07.png "7) )를 만났으므로, 스택에서 (가 나올때까지 모두 출력")
![](/media/fix/assets/SY/08.png "7) 더이상 탐색할 문자열이 없으면, 스택을 비움")
