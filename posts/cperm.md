...
* **$k$가 $a$에 대응되지 않는다.**
  ...
  * $k$는 $a$에 대응되면 **안 되는** 상황이다. <mark>$k$를 마치 $a$인 것처럼 취급하면,</mark> 이미 $k$를 선택한 $a$를 제외하고 나머지 $n-1$개의 완전 순열을 구하는 문제로 환원된다.
