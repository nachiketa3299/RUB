...
이렇게 정렬되지 않은 수열에서 뽑으려고 하면, `std::next_permutation`이 순열 끝에 도달했다고 판단해버리고 더이상 진행을 안 하기 떄문에, 원하는 모든 쌍을 뽑을 수가 없다.

이걸 해결하는 법은 두 가지가 있는데,
1. 수열 자체를 오름차순으로 정렬한다.
2. 오름차순으로 정렬하기 싫으면, 인덱스의 순서를 기억하는 수열을 새로 만든다.
...
코드로 이해하는 것이 더 빠르다: 수열 `{2, 1, 3}` 2개를 뽑는 예제이다.
...
핵심은 뒤에서부터 마킹하여 오름차순을 유지하는 것.
앞에서부터 마킹한 후 `std::reverse`로 뒤집어 주어도 좋다.

`mask` 배열을 순열로 돌린 후, `mask`가 있는 부분만 출력하면 조합이 완성이다. 구체적으로 출력은 다음처럼 작동한다:
