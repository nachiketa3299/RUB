...
싱글턴 패턴은 아마 가장 유명한 디자인 패턴일 것이다. 그러면서도 가장 많이 오용되거나 남용되는 패턴이 아닐까? 
싱글턴에 대해서 검색을 많이 하는데, *"싱글턴 패턴은 안티 패턴이니 사용하지 말라"*, 같은 의견을 심심치 않게 본다.

게임 개발에는 섹시한(?) OOP 설계를 적용하기 어려운 것 같고, 이런 구조에 대한 집착은 대체로 비생산성을 낳는 것 같다.

사실, 상용 엔진들엔 싱글턴이 매우 많다. 당신이라고 쓰지 말란 법이 없다. 해결하려는 문제에 맞게, 유연하게 적용하여 게임 개발에 편의를 도모하자.
